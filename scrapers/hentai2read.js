...
async function downloadFromHentai2Read(galleryUrl) {
  const browser = await chromium.launch({ headless: true });
  const context = await browser.newContext();
  const page = await context.newPage();

  console.log(`🌐 Opening: ${galleryUrl}`);
  ...
  await browser.close();
  return { success: true, count: pageIndex - 1 };
}

module.exports = downloadFromHentai2Read;
